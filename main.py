import ssd1306
import utime
import ntptime
import network
import machine
import framebuf
import dht
#import urequests

#construct sensor
d = dht.DHT11(machine.Pin(2))

#connect to router
ap_if = network.WLAN(network.AP_IF)
ap_if.active(False)
sta_if = network.WLAN(network.STA_IF)
sta_if.active(True)
sta_if.ifconfig(('192.168.0.200','255.255.255.0','192.168.0.1','8.8.8.8'))
sta_if.connect('router SSID','password')


#Set oled display pins
hspi = machine.SPI(-1, baudrate=1000000, polarity=1, phase=0, sck=machine.Pin(14), mosi=machine.Pin(13), miso=machine.Pin(12))

#For some reason hardware SPI is not working...
#hspi = machine.SPI(1, baudrate=8000000, polarity=0, phase=0)
dc=machine.Pin(4)
res=machine.Pin(5)
cs=machine.Pin(15)
oled = ssd1306.SSD1306_SPI(128,64,hspi,dc,res,cs)

#clear screen
oled.fill(0)


def tht():
    try:
		#set time from the internet
        ntptime.settime()
        
        #set +2h timezone
        bg=utime.localtime(utime.time()+10800)
        
        #get hour and minutes from the bg var
        hour = bg[3]
        minute = bg[4]
    except:
        pass
    
    oled.fill(0)
    #show thermometer image
    buffer=bytearray(b'')
    fb = framebuf.FrameBuffer(buffer, 35, 40, framebuf.MONO_HLSB)
    oled.framebuf.blit(fb, 20, 15)

    try:
        d.measure()
    except:
        pass    

    utime.sleep(5)
    oled.text(("%d:%02d") % (hour, minute),45,0)
    oled.text(str(d.temperature())+' C', 70, 25)
    oled.text(str(d.humidity())+' %', 70, 38)
    oled.show()
    utime.sleep(15)
    
def airplane():
    oled.fill(0)
    buffer = bytearray(b'')
    fb = framebuf.FrameBuffer(buffer, 60, 34, framebuf.MONO_HLSB)
    oled.framebuf.blit(fb, 30, 0)
    oled.text('some text here',10,40)
    oled.show()
    utime.sleep(15)

while True:
    tht()
    
    #TODO will integrate tactile button to change view with other information (downloaded json through urequests lib
    #airplane()  
